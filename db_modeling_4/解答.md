...
`kmishima`さんが`penpen`さんにコードレビュー依頼を１日おきにリマインド登録した場合の処理
...
ユーザ、リマインダー、リマインダ予定テーブルそれぞれにレコードが挿入されている。
...
現在時刻からリマインド対象のidとリマインド文章、送信先ユーザ名を取得して、アプリ側でリマインドを送信する。

その後は次回リマインド時刻を更新して実行終了
...
実行予定リマインドidから、リマインド文章、送信先ユーザ名が取得できた。

その後次回リマインド予定時刻が再登録されていることも確認した。
...
完了したリマインドidを取得して、リマインド予定テーブルからDELETEし、リマインド完了テーブルにINSERTする。
